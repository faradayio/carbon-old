...
* (guid,defer) Runs an async request (so you don't immediately get the result back). The result will be stored to S3 as {http://storage.carbon.brighterplanet.com/SHA1-hexdigest-of-key+guid}[http://storage.carbon.brighterplanet.com/SHA1-hexdigest-of-key+guid]
